    ...
    >
      ...
      <div className="p-3.5 border-b border-zinc-800 flex items-center justify-between gap-3 bg-zinc-950/90 backdrop-blur-md sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={handleCopy}
            className="px-4 py-1.5 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-zinc-950 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-cyan-500/20 transition-all cursor-pointer"
          >
            {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
            <span>{copied ? 'Copied to Clipboard!' : activeTab === 'prompt' ? 'Copy Prompt' : 'Copy Payload'}</span>
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 select-text">
        {activeTab === 'prompt' ? (
          ...
          <pre className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 font-mono text-[11px] text-cyan-400 dark:text-cyan-300 leading-normal overflow-x-auto whitespace-pre-wrap">
