...
### Added
- **[2026-08-09] Natural Real-Life Scene Planner System Prompt (Standard Mode)**: Replaced standard system prompt in `GeminiProvider.ts` and `AIDirectorPanel.tsx` with 10 Core Scene Rules (Continuous Action, Realistic Human Movement, Natural Camera, Visual Description, Character/Environment Consistency, Action Continuity). Added dual JSON parsing support for `s.character` and `s.characters` array.
